...
vis.ftdetect.ignoresuffixes = {"~$", "%.orig$", "%.bak$", "%.old$", "%.new$"}
...
vis.ftdetect.filetypes = {
	...
	crontab = {ext = {"^crontab.*$"}, cmd = {"set savemethod inplace"}},
	crystal = {ext = {"%.cr$"}},
	csharp = {ext = {"%.cs$"}},
	css = {ext = {"%.css$"}, mime = {"text/x-css"}},
	cuda = {ext = {"%.cu$", "%.cuh$"}},
	dart = {ext = {"%.dart$"}},
	desktop = {ext = {"%.desktop$"}},
	diff = {ext = {"%.diff$", "%.patch$", "%.rej$"}},
	dmd = {ext = {"%.d$", "%.di$"}},
	...
	fennel = {ext = {"%.fnl$"}},
	fish = {ext = {"%.fish$"}},
	forth = {ext = {"%.forth$", "%.frt$", "%.fs$", "%.fth$"}},
	fortran = {
		ext = {
			"%.f$", "%.for$", "%.ftn$", "%.fpp$", "%.f77$", "%.f90$", "%.f95$",
			"%.f03$", "%.f08$",
		},
	},
	fsharp = {ext = {"%.fs$"}},
	fstab = {ext = {"fstab"}},
	gap = {ext = {"%.g$", "%.gd$", "%.gi$", "%.gap$"}},
	gemini = {ext = {"%.gmi"}, mime = {"text/gemini"}},
	gettext = {ext = {"%.po$", "%.pot$"}},
	gherkin = {ext = {"%.feature$"}},
	['git-commit'] = {ext = {"^COMMIT_EDITMSG$"}, cmd = {"set colorcolumn 72"}},
	['git-rebase'] = {ext = {"git%-rebase%-todo"}},
	glsl = {ext = {"%.glslf$", "%.glslv$"}},
	gnuplot = {ext = {"%.dem$", "%.plt$"}},
	go = {ext = {"%.go$"}},
	groovy = {ext = {"%.groovy$", "%.gvy$"}},
	gtkrc = {ext = {"%.gtkrc$"}},
	haskell = {ext = {"%.hs$"}, mime = {"text/x-haskell"}},
	html = {
	...
	io_lang = {ext = {"%.io$"}},
	java = {ext = {"%.bsh$", "%.java$"}},
	javascript = {ext = {"%.cjs$", "%.js$", "%.jsfl$", "%.mjs$", "%.ts$"}},
	json = {ext = {"%.json$"}, mime = {"text/x-json"}},
	jsp = {ext = {"%.jsp$"}},
	julia = {ext = {"%.jl$"}},
	latex = {
		ext = {
			"%.bbl$", "%.cls$", "%.dtx$", "%.ins$", "%.ltx$", "%.tex$",
			"%.sty$",
		},
		mime = {"text/x-tex"},
	},
	...
	logtalk = {ext = {"%.lgt$"}},
	lua = {ext = {"%.lua$"}, mime = {"text/x-lua"}},
	makefile = {
		ext = {
			"%.iface$", "%.mak$", "%.mk$", "GNUmakefile", "makefile",
			"Makefile",
		},
	...
	php = {ext = {"%.inc$", "%.php$", "%.php3$", "%.php4$", "%.phtml$"}},
	pico8 = {ext = {"%.p8$"}},
	pike = {ext = {"%.pike$", "%.pmod$"}},
	pkgbuild = {ext = {"^PKGBUILD$"}},
	pony = {ext = {"%.pony$"}},
	powershell = {ext = {"%.ps1$"}},
	prolog = {ext = {"%.pl$", "%.pro$", "%.prolog$"}},
	props = {ext = {"%.props$", "%.properties$"}},
	protobuf = {ext = {"%.proto$"}},
	ps = {ext = {"%.eps$", "%.ps$"}},
	pure = {ext = {"%.pure$"}},
	python = {ext = {"%.sc$", "%.py$", "%.pyw$"}, mime = {"text/x-python"}},
	reason = {ext = {"%.re$"}},
	rc = {ext = {"%.rc$", "%.es$"}},
	rebol = {ext = {"%.r$", "%.reb$"}},
	rest = {ext = {"%.rst$"}},
	...
	sass = {ext = {"%.sass$", "%.scss$"}, mime = {"text/x-sass", "text/x-scss"}},
	scala = {ext = {"%.scala$"}, mime = {"text/x-scala"}},
	scheme = {ext = {"%.rkt$", "%.sch$", "%.scm$", "%.sld$", "%.sls$", "%.ss$"}},
	...
	vb = {
		ext = {
			"%.asa$", "%.bas$", "%.ctl$", "%.dob$", "%.dsm$", "%.dsr$",
			"%.frm$", "%.pag$", "%.vb$", "%.vba$", "%.vbs$",
		},
	},
	vcard = {ext = {"%.vcf$", "%.vcard$"}},
	verilog = {ext = {"%.v$", "%.ver$", "%.sv$"}},
	vhdl = {ext = {"%.vh$", "%.vhd$", "%.vhdl$"}},
	wsf = {ext = {"%.wsf$"}},
	xs = {ext = {"%.xs$", "^%.xsin$", "^%.xsrc$"}},
	xml = {
		ext = {
			"%.dtd$", "%.plist$", "%.svg$", "%.xml$", "%.xsd$", "%.xsl$",
			"%.xslt$", "%.xul$",
		},
	},
	xtend = {ext = {"%.xtend$"}},
	yaml = {ext = {"%.yaml$", "%.yml$"}, mime = {"text/x-yaml"}},
	zig = {ext = {"%.zig$"}},
}
